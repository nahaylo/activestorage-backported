...
class ActiveStorage::Blob < ActiveRecord::Base
  ...
  class << self
    ...
    def build_after_upload(io:, filename:, content_type: nil, metadata: nil, service_name: nil)
      new.tap do |blob|
        blob.filename     = filename
        blob.content_type = content_type
        blob.metadata     = metadata
        blob.service_name = service_name.presence || service.name
    ...
    def create_after_upload!(io:, filename:, content_type: nil, metadata: nil, service_name: nil)
      build_after_upload(io: io, filename: filename, content_type: content_type, metadata: metadata, service_name: service_name).tap(&:save!)
    end
    ...
    # the signed ID.
    def create_before_direct_upload!(filename:, byte_size:, checksum:, content_type: nil, metadata: nil, service_name: nil)
      create! filename: filename, byte_size: byte_size, checksum: checksum, content_type: content_type, metadata: metadata, service_name: service_name
    end
  end
  ...
  def url(expires_in: service.url_expires_in, disposition: :inline, filename: nil, **options)
